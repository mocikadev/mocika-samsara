# install.ps1 — samsara Windows 安装脚本
# 用法：irm https://raw.githubusercontent.com/mocikadev/mocika-samsara/main/install.ps1 | iex
# 或：   $env:SAMSARA_VERSION="v0.1.0"; irm .../install.ps1 | iex
...
$Binary     = "samsara"
$Artifact   = "samsara-windows-x86_64.exe"
$InstallDir = if ($env:SAMSARA_INSTALL_DIR) { $env:SAMSARA_INSTALL_DIR } else { "$HOME\.local\bin" }
$Version    = if ($env:SAMSARA_VERSION)     { $env:SAMSARA_VERSION }     else { "latest" }
...
function Install-SamsaraSkill {
    param($SamsaraExe)
    Write-Host ""
    if ($IsZh) { msg_info "正在初始化知识库并安装配套 skill..." }
    else        { msg_info "Initializing knowledge base and installing skill..." }

    try {
        & $SamsaraExe init 2>$null
        if ($IsZh) { msg_ok "初始化完成，self-evolution skill 已安装" }
        else        { msg_ok "Initialized. self-evolution skill installed." }
    } catch {
        if ($IsZh) { msg_warn "初始化失败，请稍后手动运行：samsara init" }
        else        { msg_warn "Init failed. Run manually later: samsara init" }
    }
}

function Main {
    Write-Host ""
    if ($IsZh) { Write-Host "安装 samsara — AI Agent 知识管理 CLI" -ForegroundColor White }
    else        { Write-Host "Installing samsara — AI Agent knowledge management CLI" -ForegroundColor White }
    Write-Host ""
    ...
    Install-SamsaraSkill $Dest

    Write-Host ""
    if ($IsZh) { Write-Host "完成！运行 samsara --help 开始使用。" -ForegroundColor Green }
    else        { Write-Host "Done! Run 'samsara --help' to get started." -ForegroundColor Green }
    Write-Host ""
}
